...
по десять сообщений. По завершении обмена обеспечить корректное закрытие каналов
и удаление семафора.
*/
...
void eraseSemaphore(int sem_id) {
  ...
  if (semctl(sem_id, 1, IPC_RMID, NULL) < 0) {
    perror("Error removing semaphore: ");
    exit(EXIT_FAILURE);
  }
}
...
void parent_process(int fd, int sem_id) {
  ...
  for (int i = 0; i < MSGS_COUNT; ++i) {
    ...
    semop(sem_id, &(struct sembuf){.sem_num = 1, .sem_op = -1, .sem_flg = 1},
          1);
